    ...
    函数:
        ...
        def test(a,b):
            ...
            def c():
                a()
                b ()
新式类:以object为基类的类,推荐使用,经典类.不以object 为基类的类.不推荐使用.
在python3.x中定义类时,如果没有定义父类.会默认使用object 作为改类的基类. 
新式类和经典类在多继承时- - 会影响到方法的搜索顺利
为了保证编写的代码.能够同事在python2.x和python 3.x同事运行.
...
类方法的第一个参数应该是cls
1由哪一个类调用的方法,方法内的cls就是哪一个类的引用
2这个参数和实例方法的第一个参数是self类似
3提示使用其他的名称也可以.不过习惯使用cls
